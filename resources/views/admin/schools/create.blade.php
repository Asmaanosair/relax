...
<form class="mt-4" method="post" action="{{ route('admin.schools.store') }}" enctype="multipart/form-data">
    @csrf

    <div class="mb-3">
        <label class="label text-muted mb-1">{{ __('schol_name') }}</label>
        <input required name="school" class="form-control" type="text" />
    </div>
    ...
    <div class="mb-4">
        ...
        <div class="add-file">
            <div class="add-file__title-box">
                ...
                <input required type="file" name="image" accept="image/*" onchange="showPreview(event);" />
            </div>
        </div>
    </div>

    <button type="submit" class="btn btn-primary rounded-pill">
        {{ __('add_ns') }} <i class="uil-check-circle ms-1"></i>
    </button>
</form>
@endsection
