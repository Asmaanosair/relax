...
<header class="d-flex align-items-center justify-content-between">
    ...
    <a href="{{ route('admin.clinics.create') }}" class="btn btn-primary rounded-pill">{{ __('add_nc') }}</a>
</header>
        ...
        <tbody>
            ...
            <tr>
                <td class="d-flex align-items-center">
                    <img class="rounded-pill" src="{{ $clinic->image }}" width="45" />
                    <div class="ms-3">
                        <h6 class="mb-1">{{ __('clinic') }}</h6>
                        {{ $clinic->clinic }}
                    </div>
                </td>
                <td>
                ...
                <td>
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="actionsDropdown">
                            <li>
                                <a class="dropdown-item" href="{{ route('admin.clinics.edit', $clinic->id) }}">
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="Modal{{ $clinic->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    {{ __('del_clinic_msg_conf') }}
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                                        {{ __('cancel') }} <i class="uil uil-times"></i>
                                    </button>
                                    <form method="POST" class="form-inline" action="{{ route('admin.clinics.destroy', $clinic->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger">
                                            {{ __('del_clinic') }} <i class="uil uil-trash"></i>
                                        </button>
                                    </form>
                                </div>
                            </div>
