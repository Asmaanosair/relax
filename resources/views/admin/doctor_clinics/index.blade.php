...
<header class="d-flex align-items-center justify-content-between">
    <div>
        <h2>{{ __('doc_clinics') }}</h2>
        <p class="mb-0">Lorem ipsum, dolor sit amet consectetur adipisicing.</p>
    </div>
    <a href="{{ route('admin.doctor_clinics.create') }}" class="btn btn-primary rounded-pill">
        {{ __('add_ndc') }} <i class="uil-plus"></i>
    </a>
</header>
                ...
                <td>
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                            <li>
                                <a class="dropdown-item" href="{{ route('admin.doctor_clinics.show', $doctor->id) }}">
                                    <i class="uil uil-eye me-1"></i> {{ __('view') }}
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="{{ route('admin.doctor_clinics.edit', $doctor->id) }}">
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    {{ __('del_doc_clinic_msg_conf') }}
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                                        {{ __('cancel') }} <i class="uil uil-times"></i>
                                    </button>
                                    <form method="POST" class="form-inline" action="{{ route('admin.doctor_clinics.destroy', $doctor->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger">
                                            {{ __('del_doc_clinic') }} <i class="uil uil-trash"></i>
                                        </button>
                                    </form>
                                </div>
                            </div>
